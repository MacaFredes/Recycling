...
def generar_resultados(model, x, y, w, q, I, J, K, d, f, c_nr, phi, r,
    ...
    MODIFICADO: w[i,j] indica si la persona fue a reciclar (1) o no (0) porque encontró el contenedor repleto
    """
    ...
    try:
        ...
        # Costo no reciclar: cuando w[i,j]=0 no fue a reciclar (contenedor repleto)
        costo_no_reciclar = sum(q[i] * c_nr[i] * (1 - sum(w[i,j].x for j in J)) for i in I)
        
        print(f"\nDESGLOSE DE COSTOS:")
        print(f"  - Instalación: ${costo_instalacion:,.2f}")
        print(f"  - No Reciclar (contenedores repletos): ${costo_no_reciclar:,.2f}")
    ...
    for j in J:
        ...
        if contenedor_instalado is not None:
            try:
                # CORREGIDO: residuos_recibidos = lo efectivamente reciclado
                residuos_recibidos = sum(w[i,j].x * q[i] for i in I)
            except:
                residuos_recibidos = 0
            
            try:
                # w[i,j]=1 significa que SÍ fue a reciclar, w[i,j]=0 significa que NO fue (repleto)
                residuos_reciclados = sum(w[i,j].x * q[i] for i in I)
                # CORREGIDO: solo contar no reciclados de demandas asignadas a este sitio
                residuos_no_reciclados = sum((1 - w[i,j].x) * q[i] * x[i,j].x for i in I)
            except:
                residuos_reciclados = 0
                residuos_no_reciclados = 0
            ...
            # Utilización = residuos reciclados / capacidad
            utilizacion = (residuos_reciclados / capacidad_sitio * 100) if capacidad_sitio > 0 else 0
            ...
            sitios_activados.append({
                ...
                'residuos_reciclados': residuos_reciclados,
                'residuos_no_reciclados': residuos_no_reciclados,
    ...
    # CORREGIDO: Calcular residuos no reciclados directamente desde los datos
    total_no_reciclados = sum(q[i] * (1 - sum(w[i,j].x for j in J)) for i in I)
    ...
    estadisticas = {
        ...
        'residuos_no_reciclados': total_no_reciclados,
        'tasa_reciclaje': 0,
    ...
    with PdfPages(nombre_archivo) as pdf:
        ...
        _generar_mapa_no_reciclados(pdf, df_sitios, df_candidatos)
    ...
    return (estadisticas['funcion_objetivo'], df_sitios, estadisticas['residuos_no_reciclados'], estadisticas)
...
------------------------------------------------------------------
   ...
   No Reciclado:         {stats['residuos_no_reciclados']:>12,.2f} kg
...
"""
    ...
    if len(df_sitios) > 0:
        sitio_max = df_sitios.loc[df_sitios['residuos_reciclados'].idxmax(), 'sitio']
        max_carga = df_sitios['residuos_reciclados'].max()
...
def _generar_mapa_no_reciclados(pdf, df_sitios, df_candidatos):
    """Pagina 3: Residuos No Reciclados (contenedores repletos)"""
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 8))
    fig.suptitle('ANÁLISIS DE RESIDUOS NO RECICLADOS (Contenedores Repletos)', fontsize=14, fontweight='bold')
    
    ax1.scatter(df_candidatos['longitud'], df_candidatos['latitud'],
                c='lightgray', s=50, alpha=0.4, label='No activados')
    
    if len(df_sitios) > 0:
        df_sin = df_sitios[df_sitios['residuos_no_reciclados'] < 0.01]
        df_con = df_sitios[df_sitios['residuos_no_reciclados'] >= 0.01]
        
        if len(df_sin) > 0:
            ax1.scatter(df_sin['longitud'], df_sin['latitud'],
                       c='green', s=200, alpha=0.6,
                       label=f'Sin rechazos ({len(df_sin)})', zorder=5)
        
        if len(df_con) > 0:
            ax1.scatter(df_con['longitud'], df_con['latitud'],
                       c='red', s=200, alpha=0.8,
                       label=f'Con rechazos ({len(df_con)})', zorder=5)
    
    ax1.set_xlabel('Longitud')
    ax1.set_ylabel('Latitud')
    ax1.set_title('Mapa de Contenedores con Rechazos')
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    
    ax2.axis('off')
    stats_text = "ESTADÍSTICAS DE RECHAZOS POR CAPACIDAD\n\n"
    if len(df_sitios) > 0:
        total_no_reciclados = df_sitios['residuos_no_reciclados'].sum()
        sitios_con_rechazos = len(df_sitios[df_sitios['residuos_no_reciclados'] >= 0.01])
        stats_text += f"Total no reciclado: {total_no_reciclados:.2f} kg\n"
        stats_text += f"Sitios con rechazos: {sitios_con_rechazos}\n\n"
        stats_text += "Personas que encontraron contenedores\nrepletos y no pudieron reciclar.\n"
...
def generar_excel_detallado(df_sitios, df_asignaciones, x, y, w, q, I, J, K, d, 
    ...
    with pd.ExcelWriter(nombre_archivo, engine='openpyxl') as writer:
        ...
        resumen_data = {
            ...
            'Valor': [
                ...
                f"{estadisticas['residuos_no_reciclados']:,.2f}",
        ...
        for j in J:
            for k in K:
                try:
                    if y[j,k].x > 0.5:
                        # CORREGIDO: Total recibidos = suma de lo efectivamente reciclado
                        residuos_recibidos = sum(w[i,j].x * q[i] for i in I)
                        capacidad = d[j,k]
                        ...
                        if demandas_asignadas:
                            for i in demandas_asignadas:
                                fue_a_reciclar = w[i,j].x if (i, j) in w else 0
                                detalle_candidatos.append({
                                    'Candidato': j,
                                    'Candidato_Activo': 'SI',
                                    'Demanda_Asignada': i,
                                    'Residuos_Demanda': round(q[i], 2),
                                    'Fue_a_Reciclar': round(fue_a_reciclar, 4),
                                    'Reciclados_kg': round(fue_a_reciclar * q[i], 2),
                                    'No_Reciclados_kg': round((1 - fue_a_reciclar) * q[i], 2),
                                    'Tipo_Contenedor': k,
        ...
        # ====================================================================
        # HOJA 3: RECHAZOS POR CAPACIDAD
        # ====================================================================
        
        rechazos_data = []
        
        for i in I:
            # Calcular total de residuos NO reciclados por contenedor repleto
            total_no_reciclado_i = q[i] * (1 - sum(w[i,j].x for j in J))
            
            # Solo incluir si hay residuos no reciclados
            if total_no_reciclado_i > 0.01:
                # Calcular también lo reciclado
                total_reciclado_i = q[i] * sum(w[i,j].x for j in J)
                ...
                for j in J:
                    if x[i,j].x > 0.5:
                        sitios_asignados.append(j)
                        if w[i,j].x > 0.5:
                            fue_a_reciclar.append(f"{j}: SÍ ({w[i,j].x:.2%})")
                        else:
                            fue_a_reciclar.append(f"{j}: NO ({w[i,j].x:.2%})")
                ...
                rechazos_data.append({
                    'Sitio_Demanda': i,
                    'Sitios_Asignados': ', '.join(map(str, sitios_asignados)) if sitios_asignados else 'Ninguno',
                    'Fue_a_Reciclar': ', '.join(fue_a_reciclar) if fue_a_reciclar else 'N/A',
                    'No_Reciclado_kg': round(total_no_reciclado_i, 2),
                    'Reciclado_kg': round(total_reciclado_i, 2),
                    ...
                    'Razon': 'Contenedor repleto'
                })
        
        df_rechazos = pd.DataFrame(rechazos_data)
        
        if len(df_rechazos) > 0:
            df_rechazos = df_rechazos.sort_values('No_Reciclado_kg', ascending=False)
            
            total_row = pd.DataFrame({
                'Sitio_Demanda': ['TOTAL'],
                'Sitios_Asignados': [''],
                'Fue_a_Reciclar': [''],
                'No_Reciclado_kg': [df_rechazos['No_Reciclado_kg'].sum()],
                'Reciclado_kg': [df_rechazos['Reciclado_kg'].sum()],
                'Demanda_Original_kg': [df_rechazos['Demanda_Original_kg'].sum()],
                'Sitios_Candidatos': [''],
                'Sitios_Ocupados': [''],
                'Num_Candidatos': [''],
                'Num_Ocupados': [''],
            ...
            df_rechazos = pd.concat([df_rechazos, total_row], ignore_index=True)
        
        df_rechazos.to_excel(writer, sheet_name='Rechazos_Capacidad', index=False)
    ...
    # ====================================================================
    print(f"\nVERIFICACIÓN DE CONSISTENCIA:")
    print(f"  NO Reciclados en Resumen: {estadisticas['residuos_no_reciclados']:.2f} kg")
    if len(df_rechazos) > 0:
        total_hoja3 = df_rechazos[df_rechazos['Sitio_Demanda'] == 'TOTAL']['No_Reciclado_kg'].values[0]
        print(f"  NO Reciclados en Hoja 3:  {total_hoja3:.2f} kg")
        diferencia = abs(estadisticas['residuos_no_reciclados'] - total_hoja3)
        if diferencia < 0.01:
...
if __name__ == "__main__":
    ...
    print("Uso: F_O, df_sitios, no_reciclados, stats = generar_resultados(...)")