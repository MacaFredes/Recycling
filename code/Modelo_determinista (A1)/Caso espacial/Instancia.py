...
# Rango de costos para Chile (Kyklos 2024): 44-67 USD/ton

np.random.seed(42)

# Generar costos en USD/ton
#c_ton = {j: round(np.random.uniform(44, 67), 2) for j in J} 3.3-350
c_ton = {i: 177 for i in I}


# Convertir a USD/kg (1 ton = 1000 kg)
c_nr = {i: round(c_ton[i] / 1000, 3) for i in I}  # 3 decimales para precisión
...
# k=2: 442 × (900/600)^0.5 = 442 × 1.225 = 541 USD  
# k=3: 442 × (1200/600)^0.5 = 442 × 1.414 = 625 USD
...
costos_fijos_amortizados = {
    ...
    2: 541 / VIDA_UTIL_SEMANAS,  # ~2.08 USD/semana
    3: 625 / VIDA_UTIL_SEMANAS   # ~2.40 USD/semana
}
...
# CÁLCULO BASADO EN COSTOS REALES DE MANO DE OBRA EN CHILE
#
# Fuentes:
# 1. Indeed Chile (2025): Sueldo personal de aseo
#    Link: https://cl.indeed.com/career/personal-de-aseo/salaries
#    Dato: 418,702 CLP/mes promedio ≈ 465 USD/mes
#
# 3. Kyklos Chile (2024): Costo gestión normal
#    Link: https://kyklos.cl/el-costo-economico-y-social-de-la-basura/
#    Dato: 44-67 USD/ton para gestión normal
#
# Cálculo de costo hora:
# - Sueldo: 418,702 CLP/mes ÷ 900 = 465 USD/mes
# - Jornada: 176 horas/mes (44 hrs/semana × 4 semanas)
# - Costo hora: 465 / 176 = 2.64 USD/hora
#
# Escenario: Limpieza de 100 kg de vidrio disperso
# Componentes:
# 1. Mano de obra (2 personas, 1 hora): 2 × 2.64 = 5.28 USD
# 2. Equipo (escobas, palas, bolsas): 2.00 USD
# 3. Transporte adicional: 3.00 USD
# TOTAL: 10.28 USD / 100 kg = 0.103 USD/kg
#
# Este costo es INDEPENDIENTE del tiempo (es por kg, no por semana)
# Por lo tanto: c_dump = c_j + 0.103 USD/kg

COSTO_ADICIONAL_OVERFLOW = (177*1.10)/1000  # USD/kg (independiente del tiempo)

import numpy as np
np.random.seed(42)

# Crear c_dump = c_j + costo adicional fijo
c_dump = {}
for j in J:
...
p = round(len(J)/10)*2.4 #máximo valor del contenedor




print("\n" + "=" * 80)
print("INSTANCIAS GENERADAS")  #####ESTE IMPORTA
print("=" * 80)
